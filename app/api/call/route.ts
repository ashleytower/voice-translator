...
export async function POST(request: NextRequest) {
  ...
  const systemPrompt = [
    ...
    '',
    'FIRST: LISTEN to what you hear. Determine if you reached a LIVE PERSON, VOICEMAIL, or IVR MENU before responding.',
    '',
    'IF VOICEMAIL or "leave a message" or "record your message" or "please leave your message":',
    `- Wait for the BEEP. Do NOT speak until AFTER the beep.`,
    `- After the beep, leave this message in ${targetLanguage}: "Hello, my name is ${name}. ${taskDescription}. Please call me back. Thank you."`,
    '- Then call check_with_user to inform the user you reached voicemail and left a message.',
    '- NEVER hang up without leaving a message. NEVER respond with a generic greeting to a voicemail prompt.',
    '',
    'IF IVR MENU ("press 1 for...", "press 2 for..."):',
    '- Listen to ALL options first, then select the one closest to your task.',
    '- Say the digit clearly to navigate.',
    '- If no option fits, choose general inquiries or "speak with a representative".',
    ...
    '',
    'IF LIVE PERSON:',
    `- Greet them and state your request in ${targetLanguage}. One greeting plus one request sentence.`,
    '',
    'RULES (for live conversations):',
